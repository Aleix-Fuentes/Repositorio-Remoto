...
int main(int argc, char *argv[])
{
	...
	if (conn==NULL) {
		printf ("Error al crear la conexiï¿¯ï¾¿ï¾³n: %u %s\n", 
...
/*						}*/
			...
			if (codigo ==0) //petici?n de desconexi?n
				terminar=1;
			else if (codigo ==1) //usuario contraseï¿±a
			{
/*				char user[20];*/
/*				char password[20];*/
/*				char consulta[80];*/
...
/*					}*/
			...
			else if (codigo==5) //Loguearse		2/Nombre/ContraseÃ±a
			{
				char usuarios[60];
				char contrasena [60];
				p = strtok(NULL, "/");
				...
				sprintf (consulta,"SELECT JUGADOR.USUARIO FROM JUGADOR WHERE JUGADOR.USUARIO = '%s' AND  JUGADOR.CONTRASEÑA = '%s';", usuarios, contrasena);
